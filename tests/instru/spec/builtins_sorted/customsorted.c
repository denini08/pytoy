...
static PyMethodDef CustomsortedMethods[] = {
...
static int customsorted_traverse(PyObject *m, visitproc visit, void *arg) {
    Py_VISIT(sorted_func);
    return 0;
}

static int customsorted_clear(PyObject *m) {
    Py_CLEAR(sorted_func);
    return 0;
}

static void customsorted_free(void *m) {
    customsorted_clear((PyObject *)m);
}

// Module definition
static struct PyModuleDef customsortedmodule = {
    ...
    CustomsortedMethods,
    NULL,  // Reload
    customsorted_traverse,
    customsorted_clear,
    customsorted_free
};

// Initialization function for this module
PyMODINIT_FUNC PyInit_customsorted(void) {
    PyObject *m = PyModule_Create(&customsortedmodule);
    ...
    PyObject *sorted_wrapper_func = PyCFunction_New(&CustomsortedMethods[0], NULL);
